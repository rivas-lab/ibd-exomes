...
mt = hl.read_matrix_table(
    "gs://ibd-exomes/v36meta/ice.v36+ccdg_varnarrow_autosomal_gnomad_sampleqc_gt-variantqc.mt",
    overwrite=True,
)
...
mt = mt.annotate_rows(
    goodAlleleBalance=hl.agg.fraction(
        ((mt.GT.is_hom_ref()) & (mt.AB < 0.3))
        | (mt.GT.is_het() & (mt.AB >= 0.3) & (mt.AB <= 0.7))
        | (mt.GT.is_hom_var() & (mt.AB > 0.7))
    )
)
