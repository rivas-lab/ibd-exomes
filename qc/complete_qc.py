...
vcf = hl.import_vcf(
    "gs://ibd-exomes/v36meta/raw_vcfs/*",
    force_bgz=True,
...
ccdg = hl.import_vcf(
    "gs://ibd-exomes/v36meta/ccdg.vcf.bgz", reference_genome="GRCh38", force_bgz=True
)
print(ccdg.count())
print("Writing to file...")
...
nfe_ht = hl.read_table(
    "gs://daly-finland-konrad/sum_stats_enrichments/fin_enriched_exomes_37.ht/"
)
...
mt3 = hl.read_table(
    "gs://gnomad-public/release/2.1.1/liftover_grch38/ht/genomes/gnomad.genomes.r2.1.1.sites.liftover_grch38.ht/"
)
...
mt = mt.annotate_rows(
    GEFilter=(mt.gnomad_exomes.filters.length() > 0)
    | mt.gnomad_exomes.filters.contains("RF")
)
mt = mt.annotate_rows(
    GGFilter=(mt.gnomad_genomes.filters.length() > 0)
    | mt.gnomad_genomes.filters.contains("RF")
)
...
mt = mt.checkpoint(
    "gs://ibd-exomes/v36meta/v36_varnarrow_autosomal_gnomad.mt", overwrite=True
)
print(mt.count())
...
agilent = hl.import_table(
    "gs://ibd-exomes/v36meta/agilent_samples_removed.txt",
    no_header=True,
    delimiter="\t",
)
...
mt = mt.checkpoint(
    "gs://ibd-exomes/v36meta/v36_varnarrow_autosomal_gnomad_sampleqc.mt", overwrite=True
)
print(mt.count())
...
mt = mt.checkpoint(
    "gs://ibd-exomes/v36meta/v36_varnarrow_autosomal_gnomad_sampleqc_gt-variantqc.mt",
    overwrite=True,
)

# Filters for different calls in gnoMAD exomes and genomes######
gnomad_filters = hl.import_table(
    "gs://ibd-exomes/v36meta/gnomadfilters.tsv",
    types={
        "p_value": hl.tfloat64,
        "odds_ratio": hl.tfloat64,
        "genomes_AF_NFE": hl.tfloat64,
        "exomes_AF_NFE": hl.tfloat64,
        "V": hl.tstr,
    },
)
...
gnomad_filters = gnomad_filters.annotate(
    CHROM=gnomad_filters.V.split(":")[0],
    POS=hl.int(gnomad_filters.V.split(":")[1]),
    REF=gnomad_filters.V.split(":")[2],
    ALT=gnomad_filters.V.split(":")[3],
)
gnomad_filters = gnomad_filters.annotate(
    locus=hl.locus(gnomad_filters.CHROM, gnomad_filters.POS),
    alleles=hl.array([gnomad_filters.REF, gnomad_filters.ALT]),
)
gnomad_filters = gnomad_filters.annotate(
    new_locus=hl.liftover(gnomad_filters.locus, "GRCh38")
)
...
gnomad_filters = gnomad_filters.key_by(
    locus=gnomad_filters.new_locus, alleles=gnomad_filters.alleles
)
...
mt = mt.annotate_rows(
    GE_fail=(
        hl.is_defined(mt.gnomad_exome_genome_filter)
        & (
            (mt.gnomad_exome_genome_filter.p_value < 0.00001)
            & (
                (mt.gnomad_exome_genome_filter.odds_ratio < 0.6667)
                | (mt.gnomad_exome_genome_filter.odds_ratio > 1.5)
            )
        )
    )
)
...
mt = mt.checkpoint(
    "gs://ibd-exomes/v36meta/v36_varnarrow_autosomal_gnomad_sampleqc_gt-variantqc_ge.mt",
    overwrite=True,
)
...
mt = mt.annotate_rows(
    goodHetAlleleBalance=hl.agg.fraction(
        mt.GT.is_hom_ref()
        | (mt.GT.is_het() & ((mt.AB >= 0.3) & (mt.AB <= 0.7)))
        | mt.GT.is_hom_var()
    )
)
