...
#Get rid of outliers
outliers = hl.import_table('gs://ibd-exomes/v36meta/eur_outliers.tsv')
outliers = outliers.key_by('s')
mt = mt.filter_cols(hl.is_defined(outliers[mt.s]), keep=False)
print(mt.count())
...
print(mt.count())
mt = mt.checkpoint('gs://ibd-exomes/v36meta/v36+ccdg_eur_outlier_call_rate.mt', overwrite=True)
...
mt = mt.checkpoint('gs://ibd-exomes/v36meta/v36+ccdg_eur_outlier_nfe.mt', overwrite=True)
print(mt.count())
...
mt = mt.checkpoint('gs://ibd-exomes/v36meta/v36+ccdg_eur_outlier_pruned.mt', overwrite=True)
# mt = hl.read_matrix_table('gs://ibd-exomes/v36meta/v36+ccdg_eur_filtered.mt')
# print(mt.count())

# cols = mt.cols()

# cols.select().export('gs://ibd-exomes/v36meta/eur_samples_that_pca.tsv')

#PCA on the EUR samples
print("Performing PCA...")
eigenvalues, scores, loadings = hl.hwe_normalized_pca(mt.GT, k=10)

print("Exporting results...")
scores.export('gs://ibd-exomes/v36meta/eur_outlier_pca_scores.tsv.bgz')