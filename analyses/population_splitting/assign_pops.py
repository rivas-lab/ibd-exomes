...
genomes_manifest = pd.read_table(
    "manifest/IMMUNE_CCDG_WGS_MANIFEST_MAY2018.txt", dtype=str
)
...
for i, row in manifest.iterrows():
    if (manifest.at[i, "SAMPLE_ID"] in aj) and (
...
genomes_manifest = genomes_manifest[
    ["SAMPLE_ID", "RACE/ETHNICITY", "DIAGNOSIS", "COHORT"]
]

pd.concat(
    [manifest[["SAMPLE_ID", "RACE/ETHNICITY", "DIAGNOSIS", "COHORT"]], genomes_manifest]
).to_csv("v36+ccdg_pop+diagnosis.tsv", sep="\t", index=False)
